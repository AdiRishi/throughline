...
function parseFileRecord(
  ...
  if (
    ...
    throw new Error(
      `Invalid update manifest at ${sourcePath}:${lineNumber}: incomplete file entry.`,
    );
  }
  if (!Number.isSafeInteger(currentFile.size) || currentFile.size <= 0) {
    throw new Error(`Invalid update manifest at ${sourcePath}:${lineNumber}: invalid file size.`);
  }
  if (
    ...
    throw new Error(
      `Invalid update manifest at ${sourcePath}:${lineNumber}: invalid embedded block map size.`,
    );
  }
  return {
    url: currentFile.url,
...
export function parseUpdateManifest(raw: string, sourcePath: string): UpdateManifest {
  ...
  for (const [index, rawLine] of lines.entries()) {
    ...
    const fileUrlMatch = line.match(/^  - url:\s*(.+)$/u);
    if (fileUrlMatch?.[1]) {
    ...
    const fileShaMatch = line.match(/^    sha512:\s*(.+)$/u);
    if (fileShaMatch?.[1]) {
      if (currentFile === null) {
        throw new Error(
          `Invalid update manifest at ${sourcePath}:${lineNumber}: sha512 without a file entry.`,
        );
      }
    ...
    const fileSizeMatch = line.match(/^    size:\s*(\d+)$/u);
    if (fileSizeMatch?.[1]) {
      if (currentFile === null) {
        throw new Error(
          `Invalid update manifest at ${sourcePath}:${lineNumber}: size without a file entry.`,
        );
      }
    ...
    const fileBlockMapSizeMatch = line.match(/^    blockMapSize:\s*(\d+)$/u);
    ...
    if (!topLevelMatch?.[1] || topLevelMatch[2] === undefined) {
      throw new Error(
        `Invalid update manifest at ${sourcePath}:${lineNumber}: unsupported line '${line}'.`,
      );
    }
    ...
    if (key === "version") {
      if (typeof value !== "string") {
        throw new Error(
          `Invalid update manifest at ${sourcePath}:${lineNumber}: version must be a string.`,
        );
      }
      version = value;
      continue;
    ...
    if (key === "releaseDate") {
      if (typeof value !== "string") {
        throw new Error(
          `Invalid update manifest at ${sourcePath}:${lineNumber}: releaseDate must be a string.`,
        );
      }
      releaseDate = value;
      continue;
    ...
    if (key === "path" || key === "sha512") continue;
    extras[key] = value;
  }
...
function mergeExtras(
  ...
  for (const [key, value] of Object.entries(secondary)) {
    ...
    if (existing !== undefined && existing !== value) {
      throw new Error(
        `Cannot merge update manifests: conflicting '${key}' values ('${existing}' vs '${value}').`,
      );
    }
    merged[key] = value;
  }
...
export function mergeUpdateManifests(
  ...
  if (primary.version !== secondary.version) {
    throw new Error(
      `Cannot merge update manifests with different versions (${primary.version} vs ${secondary.version}).`,
    );
  }
...
function serializeScalarValue(value: UpdateManifestScalar): string {
  return typeof value === "string" ? quoteYamlString(value) : String(value);
}
