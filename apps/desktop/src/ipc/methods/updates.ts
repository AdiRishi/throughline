...
// The three action methods return the shell's answer rather than `void`: a
// renderer that cannot distinguish "refused" from "done" has no way to render
// the difference, and would show a spinner for a request that was never taken.
export const checkForUpdate = DesktopIpc.makeIpcMethod({
  ...
  handler: Effect.fn("desktop.ipc.updates.check")(function* () {
    const updater = yield* DesktopUpdater.DesktopUpdater;
    return yield* updater.check("renderer");
  }),
});
