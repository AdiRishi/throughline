...
>()("@app/desktop/shell/DesktopShellEnvironment") {}
...
const { logWarning } = makeComponentLogger("desktop-shell-environment");
...
const startMarker = (name: string) => `__THROUGHLINE_ENV_${name}_START__`;
const endMarker = (name: string) => `__THROUGHLINE_ENV_${name}_END__`;
...
const logShellEnvironmentCommandError = (
  ...
  logWarning("shell environment probe failed", {
    errorType: error._tag,
    probe: error.probe,
    argumentCount: error.argumentCount,
    ...(error instanceof DesktopShellEnvironmentCommandTimeoutError
