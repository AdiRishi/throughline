...
const { appFocusMock, browserWindowMock, getAllWindowsMock, getFocusedWindowMock } = vi.hoisted(
  () => ({
    appFocusMock: vi.fn<(options: { readonly steal: boolean }) => void>(),
    browserWindowMock: vi.fn<(options: unknown) => void>(function BrowserWindowMock() {}),
    getAllWindowsMock: vi.fn<() => ReadonlyArray<Electron.BrowserWindow>>(),
