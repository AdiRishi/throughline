...
const { autoUpdaterMock } = vi.hoisted(() => ({
  autoUpdaterMock: {
    ...
    downloadUpdate: vi.fn<() => Promise<ReadonlyArray<string>>>(() => Promise.resolve([])),
    on: vi.fn<(eventName: string, listener: () => void) => void>(),
    quitAndInstall: vi.fn<(isSilent: boolean, isForceRunAfter: boolean) => void>(),
    removeListener: vi.fn<(eventName: string, listener: () => void) => void>(),
...
describe("ElectronUpdater", () => {
  ...
  it.effect("applies the prerelease flags a nightly channel needs", () =>
    Effect.gen(function* () {
      const updater = yield* ElectronUpdater.ElectronUpdater;
      ...
      assert.equal(yield* updater.allowDowngrade, true);
