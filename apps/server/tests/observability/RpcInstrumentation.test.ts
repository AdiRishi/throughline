...
describe("RpcInstrumentation", () => {
  it.effect("records success metrics for unary RPC handlers", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "throughline_rpc_requests_total", {
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "throughline_rpc_request_duration", {
          method: "rpc.instrumentation.success",
        }),
        true,
      );
    ...
    Effect.gen(function* () {
      ...
      const events = yield* Effect.gen(function* () {
        const fiber = yield* Stream.runCollect(
          observeRpcStream(
            "rpc.instrumentation.stream.duration",
      ...
      const snapshots = yield* Metric.snapshot;
      const snapshot = findHistogramSnapshot(snapshots, "throughline_rpc_request_duration", {
        method: "rpc.instrumentation.stream.duration",
      });
  ...
  it.effect("records spans for traced unary RPC handlers", () =>
    Effect.gen(function* () {
      const spanNames = yield* collectSpanNames(
        observeRpcEffect(
          "rpc.instrumentation.traced.unary",
          Effect.succeed("ok").pipe(Effect.withSpan("rpc.instrumentation.traced.unary.child")),
          { "rpc.aggregate": "test" },
        ),
      );

      assert.equal(spanNames.includes("ws.rpc.rpc.instrumentation.traced.unary"), true);
      assert.equal(spanNames.includes("rpc.instrumentation.traced.unary.child"), true);
    }),
  );
