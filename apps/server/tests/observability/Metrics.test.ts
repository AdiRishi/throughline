...
describe("withMetrics", () => {
  ...
  // The `dual` arm: called directly rather than through `.pipe`. A failing
  // effect must still record, tagged `outcome: "failure"`.
  ...
  // Lazy attributes exist so a label can describe what the effect *did* rather
  // than what was known before it ran; evaluating them eagerly would silently
  // record the pre-run value.
  it.effect("evaluates attributes lazily after the wrapped effect runs", () =>
    Effect.gen(function* () {
      ...
      const lazyInittedLabel = "resolved";

      yield* Effect.sync(() => {
        resolvedLabel = lazyInittedLabel;
      }).pipe(
        withMetrics({
          counter,
          timer,
  ...
  // Durations come from `Clock.currentTimeNanos`, so 1_500_000ns must land as
  // 1.5ms — a millisecond clock would floor this to 1 or 2.
