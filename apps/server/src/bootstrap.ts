 ...
 * `Option.none()` when the fd is unavailable (`EBADF`/`ENOENT`) or when no
 * line arrives before the timeout, so a missing bootstrap fd is not fatal —
 * the caller falls back to env/random tokens.
 */
...
): Effect.fn.Return<Option.Option<BootstrapEnvelope>, BootstrapError> {
