/**
 * The server's observability composition root.
 *
 * Provides, in one layer:
 *  - the logger (pretty stdout + `Logger.tracerLogger`)
 *  - the trace-level / timing references
 *  - a local file tracer writing completed spans to `serverTracePath`, with an
 *    optional OTLP delegate when a collector URL is configured
 *  - the `BrowserTraceCollector`, sharing the same sink as the local tracer so
 *    renderer spans land in the same file
 *  - optional OTLP metrics export
 *
 * The local trace file is always on; OTLP is opt-in.
 *
 * @module observability/Layers/Observability
 */
...
export const ObservabilityLive = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const tracerLayer = Layer.unwrap(
      Effect.gen(function* () {
        // One sink, two producers: the local tracer below and the browser
        // collector. Sharing it is what keeps renderer and server spans in a
        // single ordered file.
          ...
          config.otlpTracesUrl === undefined
            ...
            : yield* OtlpTracer.make({
                ...
                resource: {
                  ...
                  attributes: {
                    "service.runtime": "throughline-server",
