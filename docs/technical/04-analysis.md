...
| `journey.get`, `journey.filePatch`, `journey.fileContent`, `journey.files`, `journey.tree` | unary (immutable per journey — cacheable forever client-side; `journey.files` batches the code surface's paths)                                            |
