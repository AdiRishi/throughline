...
- **Theming**: Shiki dual themes (`{ dark, light }` + `themeType`) follow the app's existing theme plumbing (`useTheme`); trees are themed to match via CSS variables. In Electron, the renderer reads the persisted shell theme synchronously through `LocalApi` before first paint, so a local-server port change cannot reset appearance with a new browser-storage origin. A plain browser keeps its theme in `localStorage`. One accent color, as the product demands.
