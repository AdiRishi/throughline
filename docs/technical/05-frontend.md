...
- **The cluster page's middle panel is one `CodeView`** — its virtualized multi-file stack is built for exactly this. Items: one `diff` item per file in the cluster's `fileOrder`. `journey.filePatch` is parsed for the pinned Git metadata, then `processFile` receives that patch together with the old/new text from `journey.fileContent`; patch-only parsed files are partial and cannot expand context. Unchanged regions collapse by default (`expandUnchanged: false`) with in-place expansion (`expansionLineCount`) — the product's expand-context interaction is native behavior.
...
- **Changed-region navigation** — the footer's "region N of M" and the next/previous-change keystroke walk the open file's changed regions via the same `scrollTo` targets, identically in every display mode.
- **Emphasis and dimming** carry the cluster boundary: homed hunks emphasized (the product's single accent use), foreign hunks dimmed and labeled. Pierre has no per-range dim prop and each item renders inside Shadow DOM, so page CSS and `useCSSClasses` cannot target rows. A controlled `CodeView` supplies item-scoped `onPostRender` hooks that mark the virtualized line rows from the journey's hunk ranges; item `unsafeCSS` styles those markers. Annotation renderers supply the persistent textual home/resurfacing labels. The hook reapplies markers on mount/update and removes them on unmount so recycled rows never retain another file's ownership.
...
- **The file tree** is a `FileTree` model over the full head-revision path list (`journey.tree`) — the project's real tree, per the product. Changed-file markers use the built-in git-status lane (`setGitStatus`); the single `renderRowDecoration` slot carries only the most useful textual status (multiple-home count or read state, chosen in that order), while the changed-files filter is `resetPaths` between full and changed-only sets; the pinned overview entry renders above the tree in the header slot.
- **Theming and native chrome**: Shiki dual themes (`{ dark, light }` + `themeType`) follow the app's existing theme plumbing (`useTheme`); trees are themed to match via CSS variables. In Electron, the renderer reads the persisted shell theme synchronously through `LocalApi` before first paint, while the shell keeps the native window background and titlebar symbols in sync with system-theme changes. macOS fullscreen state removes the traffic-light inset, and Windows/Linux reserve the live window-controls-overlay geometry, so native controls never cover the product titlebar. A local-server port change cannot reset appearance with a new browser-storage origin. A plain browser keeps its theme in `localStorage`. One accent color, as the product demands.
...
**Settings** — detected harnesses with install/auth state (`harness.status`) and the active-harness selection (`settings.update`), plus theme. The updater section consumes one pushed, schema-validated `DesktopUpdateState`: current and available versions, stable/nightly channel, check status, download progress, recoverable failures, and the downloaded/install-ready state. Check, download, and install calls return action results as well as publishing state, so the initiating window can reconcile immediately without racing the push event. The same `LocalApi` methods return an explicit disabled state in a plain browser, while Settings omits desktop-only update controls just as it omits Diagnostics. On macOS the native **Check for Updates…** command drives the same updater module and reports unavailable, current, failed, and available outcomes through native dialogs.

The desktop host also adds a quiet Diagnostics section whose **Open logs folder** action creates the configured log directory when necessary and opens that trusted local path through `LocalApi`; the plain-browser build omits the section because it has no native file-manager capability. Calm, one page; T3 Code's provider settings page is the shape reference ([04](./04-analysis.md)).
