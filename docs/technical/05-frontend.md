...
Two conventions the per-pull-request state depends on, both learned the hard way:

- **A group of atoms is memoized with `atomBundle`, never `Atom.family`.** `Atom.family` holds each entry in a `WeakRef`, which is right when the value is an atom — the registry keeps live atoms reachable — and wrong when the value is an object _containing_ atoms, since nothing holds that object between renders. One garbage collection and the next caller gets a whole new set of atoms with new subscriptions, while the old ones keep the answers: the request succeeds on the wire and the screen waits on its loader forever. `state/bundle.ts` is a strong `Map`, and a test pins it.
- **Re-fetching is a dependency, not a call.** The journey atom depends on the completed-run id, so a finished analysis re-asks for the journey by itself. Refreshing a stream-backed atom imperatively resets it to `Initial` without restarting the stream, which is the same forever-loader failure by another route.
...
- **The cluster page's middle panel is one `CodeView`** — its virtualized multi-file stack is built for exactly this. Items: one `diff` item per file in the cluster's `fileOrder`, parsed via `parsePatchFiles` from `journey.filePatch` text. Unchanged regions collapse by default (`expandUnchanged: false`) with in-place expansion (`expansionLineCount`) — the product's expand-context interaction is native behavior. Diff items are built with `parseDiffFromFile` from `journey.fileContent` — the full old and new revisions materialized per run ([03](./03-github.md)) — rather than from patch text: a patch-parsed diff is _partial_, and Pierre offers no expansion on a partial diff, so building from both revisions is what makes the product's expand-context interaction native behaviour rather than something to reimplement. `journey.filePatch` remains the per-file patch on disk and the honesty trail's copy of what was analyzed.
...
- **Emphasis and dimming** carry the cluster boundary: homed hunks emphasized (the product's single accent use), foreign hunks dimmed and labeled. Pierre has no per-range dim prop, so this rides two seams together — `options.unsafeCSS` defines the appearance against Pierre's own `data-line` attributes, and `options.onPostRender` (the only per-item, per-line hook) decides which rows are which. The accent is a margin marker, never a line background: the addition/deletion colours underneath are the diff's own language. Spiked and settled in [ADR-0010](../adr/0010-emphasis-rides-pierres-post-render-hook.md), which also records the two library constraints any change here must respect (`version`-gated item updates, and partial diffs having no expansion).
