...
3. **Nothing polls on its own.** No background refresh loops. The PR list refreshes on app focus, on explicit user refresh, and at most once per minimum interval; staleness checks reuse the cached `pr()` view. An idle Throughline issues zero requests. Because refresh is rare, it must also be _loud_: a refresh that reached GitHub republishes the welcome view to every subscriber. The screen renders the stream, not the refresh's return value, so a refresh that only updated the held answer would leave a journey saying it is current after its pull request moved on.
