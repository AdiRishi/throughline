...
- **Diff materialization** happens here at ingestion time, from local git, and is written into the run world's `inputs/` directory: the full rename-aware `baseSha..headSha` patch, path-safe keyed per-file patches plus their manifest (what the renderer's diff surfaces are served from), the head-revision file tree listing, the seed-hunk index (`@app/journey/hunks` output), and the old/new content manifest for every changed file (what context expansion and the just-the-code view are served from — [05](./05-frontend.md)). Content entries are discriminated as text, image, or binary metadata; non-image binary bytes never cross RPC merely to render a placard. After this point, ingestion never consults git again. Journey _reading_ consults the clone for exactly one thing: serving files outside the changed set (free reading of any tree file), re-fetched on demand if the workspace was evicted.
