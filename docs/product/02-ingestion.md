...
- **Pasted-URL journeys follow the same freshness model.** If a saved journey's PR is outside the viewer-affiliation list, Throughline reads that PR's cached detail individually when the welcome index refreshes. When GitHub cannot return it — because access changed, the PR disappeared, or rate limiting parked GitHub — the immutable detail saved with the journey keeps the row and journey available. Fresh cached detail drives stale state when available; the pinned fallback never invents movement it cannot observe.
